...
def make_payment(request, room_id):
    room = get_object_or_404(Room, id=room_id)
    user = request.user
    student = Student.objects.get(users=user)

    # Check if the student already has a room
    if Sroom.objects.filter(stud_id=student.stud_id).exists():
        messages.error(request, "You already have a room assigned.")
        return render(request,'unique_room.html')
    ...
    if request.method == 'POST' and room.vacancy > 0:
        with transaction.atomic():
            ...
            # Create a new Sroom entry
            Sroom.objects.create(stud_id=student.stud_id, room_no=room.room_no)
